...
SELECT DISTINCT GROUP_CONCAT( DISTINCT Student.Student ) Student, GROUP_CONCAT( DISTINCT Student.Course ) Course,
	GROUP_CONCAT( DISTINCT CONCAT( BSBMKGCOM.Assessment, ' ~ ', BSBMKGCOM.Time_Submitted, ' ~ ', IFNULL(BSBMKGCOM.Grade, 'Not Graded') ) SEPARATOR '<br>' ) Tile1_Assessment,
	Progress.Completion,
	Student.Status
...
(
	...
	JOIN prefix_course c ON c.id = cm.course AND c.shortname REGEXP 'CIV_MKG_COM' AND NOT c.fullname REGEXP 'Dual'
	...
	AND TRIM(a.name) REGEXP 'Assessment - Presenting ideas and engaging with the audience - Upload'
) Student
JOIN
(
	...
	(
		...
		JOIN prefix_course c ON c.id = Student.Cid AND c.shortname REGEXP 'CIV_MKG_COM' AND NOT c.fullname REGEXP 'Dual'
...
(
	...
	JOIN prefix_grade_items gi ON gi.courseid = c.id AND gi.itemtype = 'mod'
		AND TRIM(gi.itemname) REGEXP 'Assessment - Presenting ideas and engaging with the audience - Upload'
...
) BSBMKGCOM ON Student.Enrolment_Identifier = BSBMKGCOM.Enrolment_Identifier

-- WHERE (BSBMKGCOM.Assessment IS NOT NULL)
