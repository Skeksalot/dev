/*
https://lms.upskilled.edu.au/blocks/configurable_reports/editcomp.php?id=280
*/
SELECT DISTINCT GROUP_CONCAT( DISTINCT Student.Student ) Student, GROUP_CONCAT( DISTINCT Student.Course ) Course,
	GROUP_CONCAT( DISTINCT CONCAT( BSBMGT.Assessment, ' ~ ', BSBMGT.Time_Submitted, ' ~ ', IFNULL(BSBMGT.Grade, 'Not Graded') ) SEPARATOR '<br>' ) Assessment,
	Progress.Completion,
	Student.Status
...
(
	...
	JOIN prefix_course c ON c.id = cm.course AND c.shortname REGEXP 'Dual_CIV_BA_LM_'
	...
	WHERE (	(LOWER(l.intro) REGEXP 'BSBMGT401')
			AND (LOWER(l.intro) REGEXP 'BSBMGT405')
		)
	AND NOT LOWER(l.intro) REGEXP 'instructions'
	AND TRIM(a.name) REGEXP '(Assessment|kCheck) - (Show and Provide Leadership)'
) Student
JOIN
(
	...
	(
		...
		JOIN prefix_course c ON c.id = Student.Cid AND c.shortname REGEXP 'Dual_CIV_BA_LM_'
...
(
	...
	JOIN prefix_grade_items gi ON gi.courseid = c.id AND gi.itemtype = 'mod'
		AND TRIM(gi.itemname) REGEXP '(Assessment|kCheck) - (Show and Provide Leadership)'
	...
	WHERE c.shortname REGEXP 'Dual_CIV_BA_LM_'
...
) BSBMGT ON Student.Enrolment_Identifier = BSBMGT.Enrolment_Identifier
