...
SELECT DISTINCT GROUP_CONCAT( DISTINCT Student.Student ) Student, GROUP_CONCAT( DISTINCT Student.Course ) Course,
	GROUP_CONCAT( DISTINCT CONCAT( CUAFOH501.Assessment, ' ~ ', CUAFOH501.Time_Submitted, ' ~ ', IFNULL(CUAFOH501.Grade, 'Not Graded') ) SEPARATOR '<br>' ) CUAFOH501_Assessment,
	GROUP_CONCAT( DISTINCT CONCAT( SITXMPR006.Assessment, ' ~ ', SITXMPR006.Time_Submitted, ' ~ ', IFNULL(SITXMPR006.Grade, 'Not Graded') ) SEPARATOR '<br>' ) SITXMPR006_Assessment,
	Progress.Completion,
	Student.Status
...
(
	...
	WHERE (	(LOWER(l.intro) REGEXP 'CUAFOH501')
			OR (LOWER(l.intro) REGEXP 'SITXMPR006')
		)
	AND NOT LOWER(l.intro) REGEXP 'instructions'
	AND ( TRIM(a.name) REGEXP 'Assessment - (Manage front of house services|Manage Staff Teams and Customers)'
...
(
	...
	(
		...
		JOIN prefix_course c ON c.id = Student.Cid AND c.fullname REGEXP 'Diploma of Event Management'
...
(
	...
	JOIN prefix_grade_items gi ON gi.courseid = c.id AND gi.itemtype = 'mod'
		AND TRIM(gi.itemname) REGEXP 'Assessment - (Manage front of house services|Manage Staff Teams and Customers)'
	...
	WHERE c.fullname REGEXP 'Diploma of Event Management'
...
) CUAFOH501 ON Student.Enrolment_Identifier = CUAFOH501.Enrolment_Identifier
LEFT JOIN
(
	SELECT
		CONCAT( '<a target="_new" href="%%WWWROOT%%/mod/assign/view.php', CHAR(63), 'id=', cm.id, '">', gi.itemname, '</a>' ) Assessment,
	...
	FROM prefix_course c
	JOIN prefix_grade_items gi ON gi.courseid = c.id AND gi.itemtype = 'mod'
		AND TRIM(gi.itemname) REGEXP 'Assessment - Establish and Conduct Business Relationship'
	JOIN prefix_course_modules cm ON cm.course = c.id AND cm.instance = gi.iteminstance
	JOIN prefix_assign a ON a.name LIKE gi.itemname AND a.course = c.id AND a.id = cm.instance
	JOIN prefix_assign_submission asub ON asub.assignment = a.id AND asub.latest = 1
	JOIN prefix_grade_grades gg ON gg.itemid = gi.id AND gg.userid = asub.userid
	...
	-- AND asub.timemodified > UNIX_TIMESTAMP('2019-7-01 00:00:00')
	-- AND asub.timemodified <= UNIX_TIMESTAMP('2020-1-01 00:00:00')
) SITXMPR006 ON Student.Enrolment_Identifier = SITXMPR006.Enrolment_Identifier

-- WHERE (CUAFOH501.Assessment IS NOT NULL) OR (SITXMPR006.Assessment IS NOT NULL)

GROUP BY Student.Enrolment_Identifier

-- HAVING (CUAFOH501_Assessment IS NOT NULL)
	-- OR (SITXMPR006_Assessment IS NOT NULL)