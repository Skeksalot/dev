...
(
	SELECT DISTINCT
		...
		c.idnumber Course_Identifier, c.id Course_ID, CONCAT( Student.Sid, c.id ) Enrolment_Identifier, c.shortname Course_Short, g.name Group_Name
	...
	AND g.id IS NULL
  	...
  	GROUP BY Student.Sid, c.shortname
)
UNION ALL
(
	SELECT DISTINCT 
