/*
https://lms.upskilled.edu.au/blocks/configurable_reports/viewreport.php?id=306
*/

SELECT Trainer, Student, Course, Assessment, Submit_Time, Grade, Time_Graded, Overdue, Total_Submissions, Account_Status, Enrolment_Status, Enrolment_End_Date
...
(
	SELECT DISTINCT
		...
		'' Total_Submissions,
        CASE
            WHEN student_account = 0 THEN 'Active'
            ELSE 'Suspended'
        END Account_Status,
        CASE
        ...
        CASE
            WHEN student_end IS NULL THEN ''
			WHEN student_end = 0 THEN ''
			ELSE DATE_FORMAT( FROM_UNIXTIME(student_end), '%Y-%m-%d')
        END Enrolment_End_Date
		
	FROM (
		SELECT DISTINCT e.courseid course_id, u.id student_id, CONCAT(u.firstname, ' ', u.lastname) student_name,
		u.suspended student_account, ue.status student_status, ue.timeend student_end, Groups.groupid student_groupid, Groups.name student_group, x.instanceid student_context_instanceid
		FROM prefix_enrol e
...
(
	...
	FROM (
		SELECT DISTINCT e.courseid course_id, u.id student_id, CONCAT(u.firstname, ' ', u.lastname) student_name,
		u.suspended student_account, ue.status student_status, ue.timeend student_end, g.id student_groupid, g.name student_group, x.instanceid student_context_instanceid
